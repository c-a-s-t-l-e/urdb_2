...
energy <- urdb_select |>
  ...
  select(eiaId, rateName, utilityName, sector, energyMin, energyMax, energyRateStrux, energyWeekdaySched, energyWeekendSched)
...
energy_sched <- energy |>
  select(c(eiaId, rateName, utilityName, sector, energyMin, energyMax, energyWeekdaySched, energyWeekendSched))

cols_to_expand <- c("energyWeekdaySched", "energyWeekendSched") # <-- add more

energy_sched <- energy_sched %>%
  ...
  select(-c(energyWeekdaySched, energyWeekendSched))
